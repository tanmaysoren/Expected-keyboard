...
public final class ClipboardHistoryService
{
  ...
  {
    if (_paste_callback != null && clip != null)
  ...
  ClipboardHistoryService(Context ctx)
  {
    ...
    {
      Logs.warn("Error initializing ClipboardHistoryService", t);
    }
  }
  ...
  {
    if (clip == null || clip.isEmpty() || Config.globalConfig() == null || !Config.globalConfig().clipboard_history_enabled)
      return;
    int size = _history.size();
    if (size > 0 && _history.get(0).content.equals(clip))
      return;
  ...
  void add_current_clip()
  {
    if (_cm == null)
      return;
    ClipData clip = null;
    // getPrimaryClip might throw when the keyboard is disconnected or lacking focus.
    try { clip = _cm.getPrimaryClip(); } catch (Throwable _e) {}
    if (clip == null)
      return;
    try
    {
  ...
  static final class HistoryEntry
  {
    ...
    {
      content = c;
      int historyTtlMinutes = 0;
      if (_service != null)
        historyTtlMinutes = _service.get_history_ttl_minutes();
      else if (Config.globalConfig() != null)
        historyTtlMinutes = Config.globalConfig().clipboard_history_duration;
