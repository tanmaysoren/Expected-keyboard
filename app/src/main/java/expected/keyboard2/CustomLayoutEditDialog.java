...
public class CustomLayoutEditDialog
{
  ...
  static class LayoutEntryEditText extends EditText
  {
    ...
    public LayoutEntryEditText(Context ctx)
    {
      ...
      setBackgroundColor(0xFF0F1322);
      _ln_paint = new Paint(getPaint());
      _ln_paint.setColor(0xFFA78BFA);
    ...
    protected void onDraw(Canvas canvas)
    {
      ...
      int padding = (int)(((int)Math.log10(Math.max(1, line_count)) + 1 + 1) * digit_width);
      if (padding != _prev_padding) {
        setPadding(padding + 16, 16, 16, 16);
        _prev_padding = padding;
      }
      super.onDraw(canvas);
      _ln_paint.setColor(0xFF00F0FF);
      canvas.getClipBounds(_clip_bounds);
      Layout layout = getLayout();
      if (layout == null) return;
      int offset = (int)(digit_width / 2.f) + 8;
