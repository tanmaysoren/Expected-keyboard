...
public final class KeyboardData
{
  ...
  /** The XML resource key name, e.g. "latn_qwerty_us". */
    ...
    for (; i_row <= i_row_end; i_row++)
    {
      ...
      for (; i_col <= i_col_end; i_col++)
      {
        ...
        for (; i_dir <= i_dir_end; i_dir++)
        {
          ...
          {
            ArrayList<Key> new_keys = new ArrayList<Key>(row.keys);
            new_keys.set(i_col, col.withKeyValue(i_dir, kv));
            rows.set(i_row, row.with_keys(new_keys));
            return true;
          }
        }
      }
  ...
  public KeyboardData insert_row(Row row, int i)
  {
    ...
    rows_.add(i, row.copy().updateWidth(keysWidth));
    return with_rows(rows_);
  }
  ...
  public static class Key
  {
    ...
    protected Key(KeyValue[] ks, KeyValue antic, int f, float w, float s, String i, Role role_)
    {
      ...
      indication = i;
      role = role_;
      if (keys != null && keys[0] != null && keys[4] == null) {
        if (keys[0].equals(KeyValue.ENTER)
            || (keys[0].getKind() == KeyValue.Kind.Keyevent && keys[0].getKeyevent() == android.view.KeyEvent.KEYCODE_ENTER)) {
          keys[4] = KeyValue.TOGGLE_FLOATING;
        }
      }
    }
  ...
  public final static class KeyPos
  {
    ...
    public boolean equals(Object o)
    {
      ...
      KeyPos keyPos = (KeyPos) o;
      return row == keyPos.row && col == keyPos.col && dir == keyPos.dir;
    }

    @Override
    public int hashCode()
    {
      return (row * 31 + col) * 31 + dir;
    }

    @Override
    public String toString()
    {
      return "KeyPos(row=" + row + ", col=" + col + ", dir=" + dir + ")";
    }
  }
