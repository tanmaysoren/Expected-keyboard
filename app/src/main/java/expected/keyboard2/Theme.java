...
public class Theme
{
  ...
  {
    if (_key_font == null && context != null) {
      try {
        _key_font = Typeface.createFromAsset(context.getAssets(), "special_font.ttf");
      } catch (Exception e) {
        Logs.exn("Error loading special_font.ttf", e);
      }
    }
    return _key_font;
  }
  ...
  public static final class Computed
  {
    ...
    {
      // Make sure that the layout isn't higher than the screen.
      float max_row_height = Math.max(20f, (float) config.screenHeightPixels / Math.max(1f, layout.keysHeight));
      row_height = Math.max(15f, Math.min(config.keyboard_rows_height_pixels, max_row_height));
    ...
    public static final class Key
    {
      ...
      {
        Paint p = (special_font && _key_font != null) ? _special_label_paint : _label_paint;
        if (special_font && _key_font != null) {
          p.setTypeface(_key_font);
        } else {
      ...
      {
        Paint p = (special_font && _key_font != null) ? _special_sublabel_paint : _sublabel_paint;
        if (special_font && _key_font != null) {
          p.setTypeface(_key_font);
        } else {
          p.setTypeface(Typeface.DEFAULT);
