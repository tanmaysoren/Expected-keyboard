...
public final class Pointers implements Handler.Callback
{
  ...
  public boolean handleMessage(Message msg)
  {
    ...
    {
      Pointer ptr = _ptrs.get(i);
      if (ptr != null && ptr.timeoutWhat == msg.what)
      {
        handleLongPress(ptr);
        return true;
      }
  ...
  private static final int SLIDER_REPEAT_INTERVAL_MS = 180;
  ...
  void startSliding(Pointer ptr, float x, float y, float dx, float dy, KeyValue kv)
  {
    ...
    ptr.sliding = new Sliding(x, y, kv.getSlider());
    if (_config.keyrepeat_enabled)
      restartLongPress(ptr);
  }
  ...
  public final class Sliding
  {
    ...
    public Sliding(float x, float y, KeyValue.Slider s)
    {
      last_x = x;
      last_y = y;
      slider = s;
    ...
    static final float SPEED_VERTICAL_MULT = 0.6f;
    ...
    public void onTouchMove(Pointer ptr, float x, float y)
    {
      ...
      float step_px = Math.max(30f, _config.slide_step_px * 1.5f);

      if (slider.isVertical())
      {
        d += (dy / step_px) * SPEED_VERTICAL_MULT;
        int d_ = (int)d;
        if (d_ != 0)
        {
          d -= d_;
          if (d_ < 0)
            ptr.value = KeyValue.sliderKey(KeyValue.Slider.Cursor_up, -d_);
          else
            ptr.value = KeyValue.sliderKey(KeyValue.Slider.Cursor_down, d_);
          _handler.onPointerHold(ptr.value, ptr.modifiers);
          if (_config.keyrepeat_enabled)
            restartLongPress(ptr);
        }
      ...
      else
      {
        ...
        if (d_ != 0)
        {
          ...
          if (d_ < 0)
          {
            if (slider == KeyValue.Slider.Selection_cursor_left || slider == KeyValue.Slider.Selection_cursor_right)
              ptr.value = KeyValue.sliderKey(slider, d_);
            else
              ptr.value = KeyValue.sliderKey(KeyValue.Slider.Cursor_left, -d_);
          }
          else
          {
            if (slider == KeyValue.Slider.Selection_cursor_left || slider == KeyValue.Slider.Selection_cursor_right)
              ...
              ptr.value = KeyValue.sliderKey(KeyValue.Slider.Cursor_right, d_);
          }
          _handler.onPointerHold(ptr.value, ptr.modifiers);
          if (_config.keyrepeat_enabled)
            restartLongPress(ptr);
